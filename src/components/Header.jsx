...
import { deleteUserProfile, generateAiInterview, updateUserProfile } from '../services/userService';
import { deleteCompany } from '../services/companyService';

export default function Header({ role, setRole, activeTab, setActiveTab, isAuthenticated, setIsAuthenticated, user, setMeetingUrl }) {
  ...
  const handleStartAiInterview = async () => {
    ...
    try {
      const data = await generateAiInterview(user.id);
      if (data.meeting_url) {
        setMeetingUrl(data.meeting_url);
        
        try {
          const updatedUser = { ...user, skillVerified: true };
          await updateUserProfile(user.id, updatedUser);
          window.dispatchEvent(new CustomEvent('profileUpdated', { detail: updatedUser }));
        } catch (updateError) {
          console.error("Failed to update profile after starting interview:", updateError);
        }
              ...
              <div className="absolute top-[48px] right-0 w-[280px] bg-[#282828] border border-[#333] rounded-xl shadow-2xl py-2 flex flex-col z-50">
                ...
                <div className="py-2 flex flex-col border-b border-[#333]">
                  {role === 'FOR CANDIDATE' ? (
                    <>
                      {!user?.skillVerified && (
                        <button className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-300 hover:bg-[#333] hover:text-white transition-colors text-left w-full" disabled={isStartingInterview} onClick={handleStartAiInterview}>
                          ...
                          {isStartingInterview ? 'Starting AI Interview...' : 'Get Verified (AI Interview)'}
                        </button>
                      )}
